...
void test_interp_netcdf(e *E, char *file1, char *file2){
	...
	get_field(E,"usurf_ensmean", &E->u[0][0][0]);
	...
	get_field(E,"vsurf_ensmean", &E->v[0][0][0]);
	...
	*/
	...
	srand ( time(NULL) );
	min_lon = 143.5;	max_lon = 144.0;
	min_lat = -47.25;	max_lat = -46.75;
	...
	for(k=0;k<nParticles;k++){
		...
		cart_pos[0] = R*cos(d2r(pos[1]))*cos(d2r(pos[0]));
		cart_pos[1] = R*cos(d2r(pos[1]))*sin(d2r(pos[0]));
		...
		//pos[1] = r2d(PI - acos(cart_pos[1]/(R*sin(d2r(pos[0])))));
		
		//printf("conv back: pos: %f, %f\n", pos[0],pos[1]);
		//printf("conv back: cart: %f, %f\n", cart_pos[0], cart_pos[1]);
		...
		//for(t=0;t<1000;t++){
			...
			for(i=0;i<E->ny;i++){
				for(j=0;j<E->nx;j++){
					...
					E->el[el].node_value[0] = E->u[17][i][j]; 
					E->el[el].node_value[1] = E->u[17][i][j+1]; 
					E->el[el].node_value[2] = E->u[17][i+1][j+1]; 
					E->el[el].node_value[3] = E->u[17][i+1][j];
			...
			*/		
			...
			dlon =  fabs(E->el[el].node_coord[1][0] -  E->el[el].node_coord[0][0]);
			//printf("dlon = %f (left = %f, right = %f)\n", dlon, E->el[el].node_coord[0][0], E->el[el].node_coord[1][0]);
			
			dlat =  fabs(E->el[el].node_coord[1][1] -  E->el[el].node_coord[3][1]);
			//printf("dlat = %f (left = %f, right = %f)\n", dlat, E->el[el].node_coord[1][1], E->el[el].node_coord[3][1]);
			
			// cartesian coords of the first three node coordinates
			// this is only used to calculate the element height and width in metres
			// so we can normalize the velocity data
			
			//x = R * cos(lat) * cos(lon)
			cart_pos[0] = R*cos(d2r(E->el[el].node_coord[0][1]))*cos(d2r(E->el[el].node_coord[0][0]));
			cart_pos[1] = R*cos(d2r(E->el[el].node_coord[1][1]))*cos(d2r(E->el[el].node_coord[1][0]));
			
			cell_width = fabs( cart_pos[1] - cart_pos[0] );
								
			//y = R * cos(lat) * sin(lon)	
			cart_pos[0] = R*cos(d2r(E->el[el].node_coord[1][1]))*sin(d2r(E->el[el].node_coord[1][0]));
			cart_pos[1] = R*cos(d2r(E->el[el].node_coord[2][1]))*sin(d2r(E->el[el].node_coord[2][0]));
			cell_height = fabs( cart_pos[1] - cart_pos[0] );
			...
			vel[0] = interp_value/cell_width * dlon;
			...
			for(i=0;i<E->ny;i++){
				for(j=0;j<E->nx;j++){
					...
					E->el[el].node_value[0] = E->v[17][i][j]; 
					E->el[el].node_value[1] = E->v[17][i][j+1]; 
					E->el[el].node_value[2] = E->v[17][i+1][j+1]; 
					E->el[el].node_value[3] = E->v[17][i+1][j];
			...
			*/
			el = get_owner_element(E, pos);
			interpolate_point(&E->el[el], &interp_value);
			vel[1] = interp_value/cell_height * dlat;
			
			
			// advect the partcle
			dt = 10800.0; // ofam surface fields are 3 hourly
			update_particle_position_euler(pos,vel,dt);
